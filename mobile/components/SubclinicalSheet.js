...
export default function SubclinicalSheet ({mbId}) {
    ...
    const getUser = async () => {  
        ...
        const url = 'http://192.168.1.10:3000/patient/search-subclinical-sheet?field=subclinical_sheet_medical_bill_id&value='+mbId;
...
// style={{ flex: 1,justifyContent: 'flex-start', alignItems: 'center'}}
    return (
         <View > 
                                ...
                                {/* <Card style={styles.card}> */}
                                    ...
                                    {element.subclinical_sheet_images !== undefined ?
                                        ...
                                        {element.subclinical_sheet_images.map((e) => {
                                            return <View key={e.uid} style={styles.view}>
                                                <TouchableOpacity onPress={() => handleView(e.name, element.subclinical_name)}>
                                                <Image  
                                                    source={{
                                                    uri: 'http://192.168.1.10:3000/upload/'+e.name,
            ...
            :   <View><Text>Không tìm thấy kết quả cận lâm sàng!</Text></View>
            }
           
            
                <Modal
                visible={visible} onDismiss={hideModal} style={styles.view}
                >
                    <TouchableOpacity onPress={hideModal}>
                    <Text>{title}</Text>
                    <Image  
                        source={{
                            uri: 'http://192.168.1.10:3000/upload/'+show,
                        }}
                        style={styles.show}
                        resizeMode = 'center'
