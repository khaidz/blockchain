    ...
    useEffect(() => {
        ...
        const f = async () => {
            ...
            try {
                ...
                const url = 'http://192.168.1.10:3000/patient/search-medical-bill?field=medical_bill_patient_id&value='+id;
                const result = await axios.get(url, config);
                if(result.data.success){
                    result.data.data.sort((e, f) => {return moment(new Date(f.medical_bill_created_at)) - moment(new Date(e.medical_bill_created_at))});
            ...
            { hr.length !== 0 ?
                ...
                { hr.map((element) => {
                        return (
                            ...
                            <DataTable.Cell>
                            {  
                                getFullTime(element.medical_bill_created_at)
                            }
                            </DataTable.Cell>
                            <DataTable.Cell><Button type='link' title="Xem phiếu khám" onPress={()=> viewMedicalBill(element)}></Button></DataTable.Cell>
                            </DataTable.Row>
                            )
                        })
                    }
