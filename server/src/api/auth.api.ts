...
router.get('/verifyPhone/:user_id', async (req, res)=>{
    ...
    if(result.success) return res.send({success: true, result: result, id: id});
...
router.post('/registry/patient', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const htmlContent = "<p>Xin chào "+ req.body.fullName + "</p>"
        + "<p>Địa chỉ email này đã được dùng để xác thực tài khoản Health Record của bạn.</p>"
        + "<p>Mật khẩu của bạn là: <b>"+ pass +"</b></p>"
        +" <p>Vui lòng nhấp vào link sau để xác thực: <a href='http://192.168.1.10:3000/auth/verifyEmail/"+patient.user_id+"'>http://192.168.1.10:3000/auth/verifyEmail/"+patient.user_id+"</a></p>"
        + "<p>Lưu ý: </p>"
...
router.post('/registry/admin', authentication, async (req: Request, res: Response) => {
    const pass = randomstring.generate(8);
    try {
        const admin: User = {
            user_id: 'U' + nanoid().toUpperCase(),
        ...
        + "<p>Địa chỉ email này đã được dùng để xác thực tài khoản Health Record của bạn.</p>"
        + "<p>Mật khẩu của bạn là: <b>"+ pass +"</b></p>"
        +" <p>Vui lòng nhấp vào link sau để xác thực: <a href='http://192.168.1.10:3000/auth/verifyEmail/"+admin.user_id+"'>http://192.168.1.10:3000/auth/verifyEmail/"+admin.user_id+"</a></p>"
        + "<p>Lưu ý: </p>"
        + "<p> - Sau khi xác thực thành công, hãy thay đổi mật khẩu để đảm bảo bảo mật tài khoản. </p>"
        + "<p> - Liên hệ quầy lễ tân khi cần cập nhật thông tin cá nhân nếu bạn là bệnh nhân. </p>"
        + "<p> - Đây là email tự động của hệ thống Health Record. Vui lòng không trả lời email này.</p>"
...
router.post('/registry/staff', authentication, async (req: Request, res: Response) => {
    const pass = randomstring.generate(8);
    try {
        const staff: User = {
            user_id: 'U' + nanoid().toUpperCase(),
        ...
        + "<p>Địa chỉ email này đã được dùng để xác thực tài khoản Health Record của bạn.</p>"
        + "<p>Mật khẩu của bạn là: <b>"+ pass +"</b></p>"
        +" <p>Vui lòng nhấp vào link sau để xác thực: <a href='http://192.168.1.10:3000/auth/verifyEmail/"+staff.user_id+"'>http://192.168.1.10:3000/auth/verifyEmail/"+staff.user_id+"</a></p>"
        + "<p>Lưu ý: </p>"
        + "<p> - Sau khi xác thực thành công, hãy thay đổi mật khẩu để đảm bảo bảo mật tài khoản. </p>"
        + "<p> - Liên hệ quầy lễ tân khi cần cập nhật thông tin cá nhân nếu bạn là bệnh nhân. </p>"
        + "<p> - Đây là email tự động của hệ thống Health Record. Vui lòng không trả lời email này.</p>"
...
router.post('/login-patient', async (req, res) => {
    try {
        ...
        const isCorrectPassword = await bcrypt.compare(req.body.password, user.Record.user_password);
