...
import { MedicalBill,createMedicalBill } from '../fabric/medicalbill/MedicalBill.fabric';
import { HealthRecord,createHealthRecord, queryHealthRecord, updateHealthRecord } from '../fabric/healthrecord/HealthRecord.fabric';
...
router.post('/create-health-record', authentication, async (req: Request, res: Response) => {
    try {
        ...
        const body = "<p>Kính chào quý bệnh nhân!</p>"
        + "<p>Email này đã được dùng để xác thực tạo sổ khám bệnh của bạn trên hệ thống Health Record.</p>"
        +"<p>Vui lòng nhấp vào link sau để xác thực tạo sổ khám bệnh: <a href='http://192.168.1.10:3000/patient/verify/"+hr.health_record_id+"/"+hr.health_record_patient_id+"'>http://192.168.1.10:3000/auth/verify/"+hr.health_record_id+"/"+hr.health_record_patient_id+"</a></p>"
        + "<p>Lưu ý: </p>"
...
router.get('/search-healthrecord/', async (req: Request, res: Response) => {
    ...
    try{
        ...
        if (result.length === 0) { return res.send({valid: false}); }
...
router.post('/create-medical-bill', authentication, async(req, res) => {
    try{
        ...
        const mb: MedicalBill = {
            ...
            medical_bill_health_insurance: req.body.health_record_health_insurance,
