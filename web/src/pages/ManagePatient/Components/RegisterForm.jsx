...
export default () => {
    ...
    const handleFormFinish = async (value) => {
        ...
        try {
            ...
            if (result.data.success) { setPosting(false); setSuccess(true); form.setFieldsValue([]);};
        } catch (error) {
            setPosting(false);
            // console.log(error);
        }
    ...
    return (
        <Card>
            <Tag color="processing" style={{fontSize: 14}}>Điền thông tin bên dưới để đăng ký mới</Tag>
        <Form
            autoComplete="off"
            labelAlign="left"
            {...layout}
        ...
        >
            ...
            <Form.Item name="role_name" label="Tài khoản dành cho:">
                <Select placeholder="Bệnh nhân" value='Bệnh nhân' onSelect={(value) => setRegrole(value)} disabled={posting}>
                    <Option value="Bệnh nhân" onSelect={(value) => setRegrole(value)} disabled={posting}>Bệnh nhân</Option>
                </Select>
            </Form.Item>
            <Form.Item
                hasFeedback
            ...
            >
                <Select placeholder="Nam"  disabled={posting}>
                    <Option value="Nam" disabled={posting}>Nam</Option>
                    <Option value="Nữ" disabled={posting}>Nữ</Option>
                </Select>
            </Form.Item>
            <Form.Item
                name="dateOfBirth"
            ...
            <Form.Item {...buttonCol}>
                <Space>
                    ...
                    <Button type="default" htmlType="button" disabled={posting} onClick={() => form.setFieldsValue([{}])}>
                        <RetweetOutlined />
                        Làm lại
                    </Button>
                </Space>
