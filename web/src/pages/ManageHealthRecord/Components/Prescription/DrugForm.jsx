...
export default ({ mbId }) => {
    ...
    const formFinish = async (value) => {
        ...
        let mess;
        const data = value;
        data.doctor_instruction = `Ngày uống ${form.getFieldValue(
            'drug_time1',
        )} lần. Mỗi lần uống ${form.getFieldValue(
            'drug_time2',
        )} viên. Uống vào các buổi ${check} - ${form.getFieldValue('drug_time4')}`;
        try {
    ...
    return (
        <Form labelCol={{ span: 8 }} labelAlign="left" onFinish={formFinish} form={form}>
            <Form.Item name="search" label="Tìm kiếm thuốc">
                <Input
                    disabled={loading}
                    onChange={() => {
            ...
            >
                <Select disabled={loading}>
                    {search.map((element) => {
                        return (
                            <Option key={element.drug_id} value={element.drug_name}>
                                <Button
                                    onClick={() => {
                                        validate(element.drug_id);
                                    }}
                                >
                                    {element.drug_name}
                                </Button>
            ...
            <Form.Item name="drug_dosage_form" label="Dạng thuốc">
                <Input disabled={loading}></Input>
            </Form.Item>
            <Form.Item name="drug_unit" label="Đơn vị">
                <Input disabled={loading}></Input>
            </Form.Item>
            <Form.Item name="drug_route" label="Đường dùng">
                <Input disabled={loading}></Input>
            </Form.Item>
            <Form.Item name="drug_instruction" label="Cách dùng">
                <Input disabled={loading}></Input>
            </Form.Item>
            <Form.Item name="drug_numbers" label="Số viên">
                <Input disabled={loading}></Input>
            </Form.Item>
            <Form.Item name="drug_time1" label="Lần uống mỗi ngày">
                <Select disabled={loading}>
            ...
            <Form.Item name="drug_time2" label="Mỗi lần uống (viên)">
                <Input disabled={loading}></Input>
            </Form.Item>
            <Form.Item name="drug_time3" label="Uống vào buổi">
                <Checkbox.Group options={options} onChange={onChange}></Checkbox.Group>
            </Form.Item>
            <Form.Item name="drug_time4" label="Uống trước/sau khi ăn">
                <Select disabled={loading}>
