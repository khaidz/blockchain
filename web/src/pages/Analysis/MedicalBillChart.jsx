...
export default () => {
  ...
  useEffect (() => {
    const f = async () => {
        const urlLogin = `${DEFAULT_HOST}/admin/search-medical-bill`;
        try {
            ...
            if(logins===undefined) {
              ...
              const a = [];
              logins.map((element) => {
                  const timeData = new Date(element.medical_bill_created_at);
  ...
  return <Card title={`Thống kê số phiếu khám năm ${year}`}><Line {...config} /></Card>;
};
